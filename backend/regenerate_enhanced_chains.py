#!/usr/bin/env python3
"""
Regenerate chains using the enhanced backward tracing logic and update the database
"""
...
async def regenerate_chains():
    ...
    engine = create_async_engine(
        ...
        connect_args={
            "server_settings": {
                "application_name": "enhanced_chain_regeneration",
            },
            "command_timeout": 120,
        }
    )
        ...
        async with async_session() as session:
            ...
            for i, analysis in enumerate(chain_analyses):
                try:
                    ...
                    if i % 10 == 0:  # Progress logging every 10 chains
                        logger.info(f"Added {i+1}/{len(chain_analyses)} chains...")
            ...
            # Check results  
            result = await session.execute(
                select(RolledOptionsChain).where(RolledOptionsChain.user_id == user_id)
            )
            final_chains = result.scalars().all()
